...
function hus_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'hus' ),
        'next_string'     => esc_html__( '&raquo;', 'hus' ),
        'before_output'   => '<div class="hus-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'hus_pagination_defaults', $defaults )
    );
